...
int main(int argc, char **argv)
{
    ...
    /////////////////////////////////////////////
    // Plan trajectory/iiwa/EffortJointInterface_J7_controller/command
    ...
    trajectory_point p = planner.compute_rect_traj_cubic_prof(t);                  // rectilinear traj
    // trajectory_point p = planner.compute_circ_traj_cubic_prof(t);               // circular traj
    /////////////////////////////////////////////
    ...
    while (ros::ok())
    {
        ...
        if (robot_state_available && aruco_pose_available){
            ...
            // Extract desired posedouble error;
            des_cart_vel = KDL::Twist::Zero();
            des_cart_acc = KDL::Twist::Zero();
            if (t <= init_time_slot){
                p = planner.compute_rect_traj_cubic_prof(0.0);      // rectilinear traj
                // p = planner.compute_circ_traj_cubic_prof(0.0);    // circular traj
            }
            else if(t > init_time_slot && t <= traj_duration + init_time_slot){
                p = planner.compute_rect_traj_cubic_prof(t-init_time_slot);        // rectilinear traj
                // p = planner.compute_circ_traj_cubic_prof(t-init_time_slot);     // circular traj
